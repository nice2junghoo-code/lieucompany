...
const SERVICES = [
  ...
  { key: 'demo', label: '부자로스터 시현 신청', image: serviceDemoImage, to: '/purchase' },
...
function ServiceSection() {
  return (
    <section className="bg-canvas px-6 pt-16 pb-16 lg:px-10 lg:pt-20 lg:pb-20">
      <div className="mx-auto grid max-w-[1400px] grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {SERVICES.map((service) => {
          const cardClassName = 'group relative block aspect-square w-full overflow-hidden rounded-md'
          const cardContent = (
            <>
              {service.image ? (
                <img
          ...
          return service.to ? (
            <Link key={service.key} to={service.to} className={cardClassName}>
              {cardContent}
            </Link>
          ) : (
            <a key={service.key} href="#" onClick={(e) => e.preventDefault()} className={cardClassName}>
              {cardContent}
            </a>
          )
        })}
